...
Class Admin_model extends CI_Model {
    ...
    public function CheckPassword($aid,$pwd)
    {
        $condition = "a_id =". $aid . " AND " . "password =" . "'" . $pwd . "'";
        $this->db->where($condition);
    ...
    //Update password
    public function UpdatePassword($password,$aid)
    {
        $this->db->where('a_id', $aid);
            if($this->db->update('admin',$password))
            {
                return 1;
            }
            else
    ...
    public function EditCheckArea($areaid,$cityid,$area)
    {
        $condition = "area_id !=". $areaid . " AND " ."city_id =". $cityid . " AND " . "area_name =" . "'" . $area . "'";
        $this->db->where($condition);
        $query = $this->db->get('area');
        if ($query->num_rows() > 0){
            return 'false';
    ...
    public function UpdateArea($areaid,$area)
    {
    ...
    public function chkareapresent($areaid)
    {
    ...
    public function GetSubAdmin()
    {
        ...
        $this->db->join('city','city.city_id=sub_admin.city_id');
        $this->db->join('area','area.area_id=sub_admin.area_id');
        $this->db->where('verified',1);
        $query = $this->db->get();
        return $query->result();       
    }
    public function CheckMobileNoExistForSubAdmin($mobileno)
    {
        $condition = "mobile_no =" . "'" . $mobileno . "'";
        $this->db->where($condition);
    ...
    public function AddSubAdmin($subadmin)
    {
        if($this->db->insert('sub_admin',$subadmin))
        {
            return 1;
        }
        else
    ...
    public function EditCheckMobileNoExistForSubAdmin($said,$mobileno)
    {
        $condition = "sub_admin_id !=". $said . " AND " . "mobile_no =" . "'" . $mobileno . "'";
        $this->db->where($condition);
        $query = $this->db->get('sub_admin');
        if ($query->num_rows() > 0){
            return 'false';
    ...
    public function UpdateSubAdmin($said,$subadmin)
    {
        $this->db->where('sub_admin_id', $said);
        if($this->db->update('sub_admin',$subadmin))
        {
            return 1;
        }
        else
    ...
    public function DeleteSubAdmin($said)
	{
		$this->db->where("sub_admin_id",$said);
    ...
    public function BlockUnblockSubAdmin($said,$blockunblockvalue)
    {
        $this->db->where('sub_admin_id', $said);
   ...
   /* public function setcheckitemsnoti($anoti)    
    ...
    public function CheckShippingChargePresent($range)
    {
        $this->db->select('*');
        $this->db->from('shipping');
    ...
    public function UpdateShippingCharges($shippingid,$data)
    {
    ...
    public function EditCheckShippingChargePresent($shippingid,$range)
    {
        $this->db->select('*');
        $this->db->from('shipping');
        $this->db->where('shipping_id!=',$shippingid);
